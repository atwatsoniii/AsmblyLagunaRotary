...
allowedCircularPlanes = 0; // allow XY plane only
...
function onRapid(_x, _y, _z) {
  ...
  if (x || z) {
    ...
    writeBlock(gMotionModal.format(0), x,/* y, */z);//NN removed Y
    forceFeed();
  }
}
...
function onRapid5D(_x, _y, _z, _a, _b, _c) {
  ...
  if (currentSection.isOptimizedForMachine()) {
    var x = xOutput.format(_x);
    var y = aOutput.format(_a); //NN changed y to a output
    var z = zOutput.format(_z);
    var a = yOutput.format(_y); //NN changed a to y output
    var b = bOutput.format(_b);
    var c = cOutput.format(_c);
    writeBlock(gMotionModal.format(0), x, y, z,/* a, */b, c); //NN removed a
  } else {
    forceXYZ();
    var x = xOutput.format(_x);
    var y = yOutput.format(_y);
